...
fun ItemEntryBody(
    ...
    LaunchedEffect(Unit) {
        if (itemUiState.itemDetails.timestamp == 0L) {
    ...
    // Set calendar to current timestamp
    calendar.timeInMillis = itemUiState.itemDetails.timestamp.takeIf { it > 0 } ?: System.currentTimeMillis()
...
fun ItemInputForm(
    ...
    Column(
        ...
        // GFAP Value field - centered again
        OutlinedTextField(
            value = itemDetails.gfapval,
            onValueChange = { onValueChange(itemDetails.copy(gfapval = it)) },
            keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Decimal),
            label = { Text(stringResource(R.string.gfap_value_req)) },
            ...
            modifier = Modifier
                .fillMaxWidth(0.5f)
                .aspectRatio(3.125f)
                .align(alignment = Alignment.CenterHorizontally), // Center horizontally
            enabled = enabled,
            singleLine = true
        )
        
        // Comment field - transparent with just an underline
        TextField(
            value = itemDetails.comment,
            onValueChange = { onValueChange(itemDetails.copy(comment = it)) },
